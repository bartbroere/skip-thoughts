...
def load_tables(**k):
    ...
    """
    words = []
    utable = numpy.load(path_to_tables + 'utable.npy', **k)
    btable = numpy.load(path_to_tables + 'btable.npy', **k)
...
def encode(model, X, use_norm=True, verbose=True, batch_size=128, use_eos=False):
    ...
    for k in ds.keys():
        if verbose:
            print(k)
        if six.PY2: numbatches = len(ds[k]) / batch_size + 1
        if six.PY3: numbatches = len(ds[k]) // batch_size + 1
...
def load_params(path, params, **k):
    """
    load parameters
    """
    pp = numpy.load(path, **k)
